...
ent="mod"
init="normal"
nm = "{}-RY-layers-{}||lr-{}||entanglement-{}||basis_change-{}||init-{}||batch-{}||episodes-{}".format(init,n_layers,lr_q,ent,basis_change,init,batch_size,episodes)
wandb.init(name=nm,project="acrobot")#, entity="quantumai")
...
def qcircuit(inputs, weights0):
    ...
    return [expectation(Z(0)), expectation(Z(1)), expectation(Z(2))]# @ Z(1) @ Z(2))]
...
class policy_estimator_q(nn.Module):        
    def __init__(self, env):
        ...
        self.glorot = functools.partial(torch.nn.init.normal_, mean=0.0, std=np.sqrt(2/7))
